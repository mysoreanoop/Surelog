...
std::list <std::string> nets, ternaries, cases, ifs;
...
std::string visitLeaf(vpiHandle h) {
  ...
  switch(((const uhdm_handle *)h)->type) {
    ...
    case UHDM::uhdmhier_path : //Will never match??
...
int main(int argc, const char** argv) {
  ...
  std::ofstream f ("control_signals");
  ...
  std::cout << "\n\n\n*** Printing if conditions ***\n\n\n";
  for(it = ifs.begin(); it != ifs.end(); ++it) {
      std::cout << *it << std::endl;
      f << *it << std::endl;
  }
  std::cout << "\n\n\n*** Printing ternary conditions ***\n\n\n";
  for(it = ternaries.begin(); it != ternaries.end(); ++it) {
      std::cout << *it << std::endl;
      f << *it << std::endl;
  }
