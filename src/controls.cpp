...
std::string visitCond(vpiHandle);
std::string visitLeaf(vpiHandle);
...
std::list <std::string> all, nets, ternaries, cases, ifs;

void print_list(std::list<std::string> &list, bool f = false, std::string fileName = "") {
    std::ofstream file;
...
std::string visitref_obj(vpiHandle h) {
  ...
  if(actual) {
    std::cout << "Walking reference obj; type: " << 
      std::to_string(((const uhdm_handle *)actual)->type) << std::endl;
    switch(((const uhdm_handle *)actual)->type) {
      case UHDM::uhdmenum_const :
      case UHDM::uhdmparameter : {
        std::cout << "Enum or parameter\n";
        s_vpi_value value;
      ...
      default :
        std::cout << "Actual reference object\n";
        if (const char* s = vpi_get_str(vpiFullName, actual))
          out += s;
        else if(const char *s = vpi_get_str(vpiName, actual))
          out += s;
    ...
    std::cout << "Not actual; object type: " << 
...
std::string visitbit_sel(vpiHandle h) {
  ...
  if(!par) std::cerr << "Couldn't find parent of bit_sel!\n";
...
std::string visitLeaf(vpiHandle h) {
  ...
  switch(((const uhdm_handle *)h)->type) {
    ...
    case UHDM::uhdmhier_path : //XXX redundant
      std::cout << "Struct at leaf\n";
  ...
  if(it) {
    ...
    while(vpiHandle itx = vpi_scan(it)) {
      ...
      std::cout << "Found ref object; type: " << 
        std::to_string(((const uhdm_handle *)itx)->type) << std::endl;
      if(first) {
  ...
  if(!saveVariables) {
    ...
    if(ops) {
      ...
      std::cerr << "Couldn't iterate on operands! Iterator type: " << 
        std::to_string(((const uhdm_handle *)ops)->type) << std::endl;
    }
  } else {
...
void visitTernary(vpiHandle h) {
  ...
  all.splice(ternaries.end(), current);
  return;
}
...
int main(int argc, const char** argv) {
  ...
  print_list(all, true, "control_signals");
  //std::cout << "\n\n\n*** Printing case conditions ***\n\n\n";
  //print_list(cases, true, "control_signals");
  //std::cout << "\n\n\n*** Printing if/if-else conditions ***\n\n\n";
  //print_list(ifs, true, "control_signals");
  //std::cout << "\n\n\n*** Printing ternary conditions ***\n\n\n";
  //print_list(ternaries, true, "control_signals");
